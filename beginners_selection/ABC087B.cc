...
int main(int argc, char* argv[]) {
  ...
  CIN(A, B, C, X); DBG(A, B, C, X);
  coins.pb(mp(500, A)); coins.pb(mp(100, B)); coins.pb(mp(50, C));
  sort(ALL(coins), [](const coin& lhs, const coin& rhs) { return lhs.scd < rhs.scd; });

  int count=0;
  FOR (i, 0, min(X / coins[0].fst, coins[0].scd) + 1, 1) {
    auto Y = X - coins[0].fst * i;
    FOR (j, 0, min(Y / coins[1].fst, coins[1].scd) + 1, 1) {
      if (!((Y - coins[1].fst * j) % coins[2].fst) && ((Y - coins[1].fst * j) / coins[2].fst <= coins[2].scd)) count++;
    }
  }

  COUT(count); ENDL(); fflush(stdout);
  return 0;
}
