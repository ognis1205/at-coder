/* $File: ABC086C.cc, $Timestamp: Sat Nov 28 22:59:18 2020 */
#ifdef LOCAL
#  include <iostream>
#  include <iomanip>
#  include <string>
...
vec<dir> ds = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};

bool IsSatisfiable(const int& i, const int& x) {
  REP (j, i + 1) if (x == j - (i - j)) return true;
  return false;
}

bool IsReachable(const int& i, const int& x, const int& y) {
  REP (j, i + 1) if (IsSatisfiable(j, x) && IsSatisfiable(i - j, y)) return true;
  return false;
}
...
//    return false;
//  } else if (i == 1) {
//    return find(ALL(ds), mp(x, y)) != ds.end();
//  } else {
//    TRAV (d, ds) if (IsReachable(i - 1, x + d.fst, y + d.scd)) return true;
//  }
//  return false;
//}
...
int main(int argc, char* argv[]) {
  ...
  REP (i, N) {
    int t, x, y; CIN(t, x, y);
    ts.pb(t); xs.pb(x); ys.pb(y);
  }
  DBG(N, xs, ys, ts);
  ASSERT(IN(N, 1, 1e5));
  TRAV (x, xs) ASSERT(IN(x, 0, 1e5));
  TRAV (y, ys) ASSERT(IN(y, 0, 1e5));
  TRAV (t, ts) ASSERT(IN(t, 1, 1e5));
  FOR (i, 0, SIZE(ts) - 1, 1) ASSERT(ts[i] < ts[i+1]);

  ts.insert(ts.begin(), 0); xs.insert(xs.begin(), 0); ys.insert(ys.begin(), 0);
  REP (i, N) {
    if (!IsReachable(ts[i+1] - ts[i], xs[i+1] - xs[i], ys[i+1] - ys[i])) {
