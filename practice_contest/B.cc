...
bool LessThan(char l, char r) {
  if (l == r) return false;
  char res; COUT("? "); COUT(l); COUT(" "); COUT(r); ENDL(); fflush(stdout); CIN(res);
  switch (res) {
  case '<': return true;
...
using iter = vec<char>::iterator;
void SortN26(iter&& l, iter&& r) {
  if (r - l > 1) {
    auto m = l + (r - l) / 2;
    SortN26(forward<iter>(l), forward<iter>(m));
    SortN26(forward<iter>(m), forward<iter>(r));
    inplace_merge(forward<iter>(l), forward<iter>(m), forward<iter>(r), LessThan);
  }
}
...
void SortN5() {
  ...
  // x1 < y1 < y2
  x1 = as[0]; x2 = as[1];
  if (LessThan(x2, x1)) swap(x1, x2);
  y1 = as[2]; y2 = as[3];
  if (LessThan(y2, y1)) swap(y1, y2);
  if (LessThan(y1, x1)) {
    swap(x1, y1);
    swap(x2, y2);
  }
  e = as[4];
  // insert e.
  as = vec<char>{x1, y1, y2};
  auto pos = FST(0, 2, [&](const int& x) -> bool { return LessThan(e, as[x]); });
  as.insert(as.begin() + pos, e);
  // insert x2.
  pos = FST(1, 3, [&](const int& x) -> bool { return LessThan(x2, as[x]); });
  as.insert(as.begin() + pos, x2);
}
